...
class Struct extends Component {
  ...
  setRef = (name) => (ref) => {
    if (ref) {
      this.refsMap.set(name, ref);
    } else {
      this.refsMap.delete(name);
    }
  };

  isValueNully() {
    return Array.from(this.refsMap.values()).every(ref => ref.isValueNully());
  }

  removeErrors() {
    this.setState({ hasError: false });
    this.refsMap.forEach(ref => ref.removeErrors());
  }

  getValue() {
    const value = {};
    this.refsMap.forEach((ref, name) => {
...
export class List extends Component {
  ...
  setRef = (index) => (ref) => {
    if (ref) {
      this.refsMap.set(index, ref);
    } else {
      this.refsMap.delete(index);
    }
  };
  ...
  getItems() {
    ...
    return value.map((itemValue, i) => {
      ...
      //passing adjacent contextto enable dyanmic filters
      const _itemOptions = Object.assign({}, itemOptions);
      // let _itemOptions = itemOptions;
      // try {
        if(_itemOptions.fields) {
          for(fieldKey in _itemOptions.fields) {
            if(_itemOptions.fields[fieldKey]['_enum']) {
              _itemOptions.fields[fieldKey]['_enum']['groupValues'] = Object.assign({}, itemValue);
              console.log(i, _itemOptions.fields[fieldKey]['_enum']['groupValues']);
              // __itemOptions.fields[fieldKey]['_enum']['groupValues'] = itemValue;
            }
          }
        }
        const __itemOptions = getDeepCopy(_itemOptions)
      // } catch(err) {
      //   console.log(err);
      // }
      const ItemComponent = getFormComponent(itemType, __itemOptions);
      const buttons = [];
...
class Form extends React.Component {
  ...
  getComponent(path) {
    ...
    let component = this.inputRef.current;
    for (const name of path) {
      if (component && component.refsMap) {
        component = component.refsMap.get(name);
      } else {
        return null;
      }
    }
